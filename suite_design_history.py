...
def suite_design_history_html(branch, suite, flow, baseline, report,all_pkl_name, mean_pkl_name, metrics_selection, html_title):
  ...
  for metric in metrics_selection:
    ...
    # Dict of metrics for ChartJS implementation:
    des_dict_chartjs = {}
    for design, nightly_dict in des_rows.items():
      des_dict_chartjs[design] = []
      for nightly, value in nightly_dict.items():
        if value != design:
          if value == '--':
            des_dict_chartjs[design] += ['null']
          else:
            des_dict_chartjs[design] += [value]

      # quick fix to onverted values list onn the chart
      des_dict_chartjs[design].reverse()

    labels_chartjs = head_row[1:]

    #pp.pprint(labels_chartjs)

    html = template.render(
        html_title = metric,
        metric = metric,
        m_selection = metrics_selection,
        branch = branch,
        suite = suite,
        flow = flow,
        report = report,
        ...
        head_col = head_col,
        des_rows = des_rows,
        des_dict_chartjs = des_dict_chartjs,
        labels_chartjs = labels_chartjs,
        suff_title = html_title,
        )
